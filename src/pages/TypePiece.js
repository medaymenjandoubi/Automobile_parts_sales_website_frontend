...
export default function TypePiece() {
    ...
    const fetchUser = async () => {
      try {
        ...
        if (response.status === 200) {
          ...
          console.log("These are all the typePiece",typePieceResponse.data)
  ...
  const handleSubmit = async () => {
    try {
      const { data } = await axios.post(`${process.env.REACT_APP_API_URL}postTypePiece`, {
        ...values, image,
      });
      ...
      setTypePiece([...typePiece, data.brand]);
  ...
  const onDrop = async (acceptedFiles) => {
    ...
    Resizer.imageFileResizer(file, 720, 500, "JPEG", 100, 0, async (uri) => {
      try {
        const { data } = await axios.post(`${process.env.REACT_APP_API_URL}upload-image`, { image: uri ,bucketName:"bousninatypepiece"}, {
          withCredentials: true,
        });
        setImage(data)
        notification.success({
  ...
  const handleDelete = async (id, image) => {
    try {
      ...
      await axios.delete(`${process.env.REACT_APP_API_URL}deleteTypePieceById/${id}`, {
        withCredentials: true,
      });

      // Update state
      setTypePiece(typePiece.filter(typePiece => typePiece._id !== id));
  ...
  return (
    <div className="flex" style={{width:"100%"}}>
      ...
      <div className="main-content">
        ...
        <Table dataSource={typePiece} columns={columns} rowKey="_id" className="mt-4" />
        <Modal
          title="Add Article"
